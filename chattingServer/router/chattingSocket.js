...
const chattingSocket = (server) => {
    ...
    io.on('connection',async (chatSocket) => {
        ...
        logger.info(`[Socket][info] | 채팅방 ID: ${headers.chattingroom}, 발신자: ${sender}, 수신자: ${headers.receiver}`);
        chatSocket.join(headers.chattingroom);
        logger.info(`${chatSocket.id} 채팅방 ${headers.chattingroom}에 참여`);
        logger.info(`[Socket][info] | ${chatSocket.id} 채팅방 ${headers.chattingroom}에 참여`);
        ...
        chatSocket.on('disconnect', async (reason) => {
            logger.info(`[Socket][disconnect] \`클라이언트 연결 해제: ${chatSocket.id}, 이유: ${reason}, roomId :${headers.chattingroom}\``);
            try {
                let roomClients = io.adapter.rooms.get(headers.chattingroom) || new Set();
                // 상대방이 방에 접속해 있는지 확인
                if(roomClients.size === 0){
                    ...
                    WHERE room_id = $1 
                    `;
                    const result = await dbService.pool.query(query, [headers.chattingroom]);
                    ...
                    if (parseInt(messageCount) === 0) {
                        ...
                        WHERE room_id = $1
                        `;
                            await dbService.pool.query(deleteQuery, [headers.chattingroom]);
                        logger.info(`[Socket][disconnect] 빈 채팅방 삭제 완료: ${headers.chattingroom}`);
                    }else{
                        logger.info(`[Socket][disconnect] 방에 메시지가 있어 삭제하지 않음: ${headers.chattingroom}`);
                    }
                }else{
                    logger.info(`[Socket][disconnect] 다른 사용자가 방에 남아있음: ${headers.chattingroom}`);
                }
            }catch (e) {
